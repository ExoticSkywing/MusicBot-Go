...
type jamendoTrackItem struct {
	...
	IsDownloadable *int `json:"isDownloadable"`
}
...
func New(ctx context.Context, cookie string, client *http.Client) *Jamendo {
	if ctx == nil {
	...
	return &Jamendo{ctx: ctx, client: client, cookie: cookie}
}
...
func (j *Jamendo) apiGet(apiURL, path string) ([]byte, error) {
	return utils.Get(j.ctx, j.client, apiURL,
...
func buildSong(item jamendoTrackItem, meta jamendoTrackMeta) *model.Song {
	if item.Status.Available != nil && !*item.Status.Available {
		return nil
	}
...
func pickBestQuality(streams map[string]string) (string, string) {
	for _, key := range []string{"flac", "mp33", "mp32", "mp3", "ogg"} {
		if url := streams[key]; url != "" && !model.ExplicitPreviewURL(url) {
			switch key {
			case "mp33", "mp32":
				return url, "mp3"
			default:
