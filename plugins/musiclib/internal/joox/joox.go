...
func New(ctx context.Context, cookie string, client *http.Client) *Joox {
	if ctx == nil {
	...
	return &Joox{ctx: ctx, client: client, cookie: cookie}
}
...
// fetchLegacySongInfo calls the legacy CGI endpoint. Keep it as a download
// fallback for installations where JOOX still serves that endpoint.
func (j *Joox) fetchLegacySongInfo(songID string) (*model.Song, error) {
	params := url.Values{}
	...
	var resp struct {
		Msong     string `json:"msong"`
		Msinger   string `json:"msinger"`
		Malbum    string `json:"malbum"`
		Img       string `json:"img"`
