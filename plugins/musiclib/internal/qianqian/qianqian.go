...
func New(ctx context.Context, cookie string, client *http.Client) *Qianqian {
	return &Qianqian{ctx: ctx, cookie: cookie, client: client}
}
...
func (q *Qianqian) fetchDownloadURL(tsid string, song *model.Song) (string, error) {
	...
	for _, rate := range qualities {
		...
		// trail_audio_info contains a preview, not the complete track.
		if downloadURL != "" {
			if model.ExplicitPreviewURL(downloadURL) || !model.CatalogDurationsMatch(song.Duration, resp.Data.Duration) {
				sawPreview = true
				continue
			}
			song.Ext = resp.Data.Format
			song.Size = resp.Data.Size
			if rate != "3000" {
				song.Bitrate, _ = strconv.Atoi(rate)
			} else {
				song.Bitrate = 0
			}
			return downloadURL, nil
		}
		if resp.Data.TrailAudioInfo.Path != "" {
			sawPreview = true
		}
	}
	if sawPreview {
		return "", fmt.Errorf("%w: qianqian returned only trail audio", model.ErrPreviewOnly)
	}
...
func (q *Qianqian) fetchSongInfo(tsid string) (*model.Song, error) {
	...
	return &model.Song{
		...
		IsVIP: item.IsVip != 0 || item.IsPaid != 0,
	}, nil
}
