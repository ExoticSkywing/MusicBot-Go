...
func TestSendMusicDirectRejectsUnverifiedAudio(t *testing.T) {
	...
	for _, fileID := range []string{"", "old-preview"} {
		if _, err := h.sendMusicDirect(context.Background(), nil, nil, &botpkg.SongInfo{FileID: fileID}, "", ""); !errors.Is(err, platform.ErrIncompleteAudio) {
			t.Fatalf("unverified audio reached delivery: %v", err)
		}
	}
}
